...
import type { TDataTableContextMenuProps } from "@/types/index";
...
const getDefaultState = (): IDataTableStore => ({ isSelecting: false, contextMenuProps: undefined});

export const createDataTableStore = (preloadedState: IDataTableStore) => {
    return createZustandStore(combine({...getDefaultState(), ...preloadedState},(set)=>({
        toggleSelection: () => {
            set(prev=>({
