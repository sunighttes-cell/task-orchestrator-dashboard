...
describe("RegisterForm", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    (useRegisterUser as any).mockReturnValue({
      mutate: mockMutate,
      isPending: false,
    });
    (usePasswordValidation as any).mockReturnValue({
      valid: true,
      rules: [],
    });
  });
  ...
  it("renders the registration form", () => {
    ...
    expect(screen.getByText(/Register as a new user/i)).toBeInTheDocument();
  ...
  it("shows validation errors when fields are empty and submitted", async () => {
    ...
    expect(await screen.findByText(/Username must be at least 5 characters/i)).toBeInTheDocument();
